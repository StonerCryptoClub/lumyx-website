...
function optimizeImages() {
  ...
  // Use imagemin for optimization (you need to install it: npm install -g imagemin-cli)
  exec(`imagemin images/**/*.{jpg,png,svg} --out-dir=${path.join(config.outputDir, 'images')}`, (error, stdout, stderr) => {
    if (error) {
      console.error(`Error optimizing images: ${error.message}`);
      console.log('You may need to install imagemin-cli: npm install -g imagemin-cli');
      return;
    }
    if (stderr) {
      console.error(`stderr: ${stderr}`);
...
function copyOtherFiles() {
  ...
  // Copy robots.txt, sitemap.xml, etc.
  ['robots.txt', 'sitemap.xml', 'favicon.ico', 'Logo.png.png'].forEach(file => {
    try {
...
// Function to copy other JavaScript files
