...
window.contentfulHelpers = {
    ...
    getCaseStudies: async () => {
        console.log('🔍 [UPDATED] Attempting to fetch case studies from Contentful...');
        try {
            ...
            if (response && response.items && response.items.length > 0) {
                ...
                console.log('📝 First case study:', response.items[0]);
                console.log('📝 First case study fields:', response.items[0].fields);
                
                // Return the real case studies (they already have the fields property)
                const realStudies = response.items;
                console.log('🔄 Returning real case studies:', realStudies.length);
                
                // Add some fallback placeholders to fill up to 3 total
                const neededPlaceholders = Math.max(0, 3 - realStudies.length);
                const finalStudies = [...realStudies, ...placeholders.slice(0, neededPlaceholders)];
                console.log('🎯 Final studies to display:', finalStudies.length);
                
                return finalStudies;
            } else {
                console.log('⚠️ No case studies found in Contentful, using placeholders');
                return placeholders;
            }
        } catch (error) {
