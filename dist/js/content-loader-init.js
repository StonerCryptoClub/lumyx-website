 ...
 * This script initializes the content loaders and ensures they have access to 
 * placeholder data in case the Contentful API fails.
 */

// Define fallback placeholder content in case the generate-placeholders.js file fails to load
window.PLACEHOLDER_CASE_STUDIES = window.PLACEHOLDER_CASE_STUDIES || [
    {
        fields: {
...
window.PLACEHOLDER_BLOG_POSTS = window.PLACEHOLDER_BLOG_POSTS || [
    {
        fields: {
...
// Initialize contentful helpers if they don't exist
if (!window.contentfulHelpers) {
    console.log('Creating fallback contentfulHelpers object');
    window.contentfulHelpers = {
        isReady: () => true,
        getPlaceholders: () => window.PLACEHOLDER_CASE_STUDIES,
        getCaseStudies: async () => window.PLACEHOLDER_CASE_STUDIES,
        getCaseStudyBySlug: async (slug) => {
            return window.PLACEHOLDER_CASE_STUDIES.find(study => study.fields.slug === slug) || window.PLACEHOLDER_CASE_STUDIES[0];
        },
        getBlogPosts: async () => window.PLACEHOLDER_BLOG_POSTS,
        getBlogPostBySlug: async (slug) => {
            return window.PLACEHOLDER_BLOG_POSTS.find(post => post.fields.slug === slug) || window.PLACEHOLDER_BLOG_POSTS[0];
        }
    };
    
    console.log('Fallback contentfulHelpers initialized');
}

console.log('Content loader initialization complete'); 